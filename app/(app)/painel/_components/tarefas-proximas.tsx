...
import { ArrowRight, Check, Clock, AlertTriangle, Repeat, GripVertical } from 'lucide-react';
        ...
        {items.map((t, i) => {
          ...
          return (
            <li
              ...
              {...drag.itemProps(i)}
              className={cn(
                'group flex flex-wrap sm:flex-nowrap items-center gap-2 sm:gap-3 px-2.5 py-2.5 rounded-lg border border-transparent hover:border-white/[0.06] hover:bg-white/[0.025] active:bg-white/[0.05] transition-all',
                atrasada && 'pulse-border-l'
              )}
            >
              {/* Drag handle — só desktop */}
              <GripVertical className="hidden lg:block w-3.5 h-3.5 text-zinc-600 opacity-0 group-hover:opacity-100 transition-opacity cursor-grab active:cursor-grabbing shrink-0" />
                ...
                <div className="flex items-center gap-1.5">
                  <span
                    className={cn(
                      ...
                      t.tipo === 'compromisso' ? 'bg-emerald-500/60' : 'bg-amber-500/60'
                    )}
                    aria-hidden
                  />
                  <span className="text-sm font-medium truncate text-zinc-100 min-w-0">
                    {t.titulo}
                  </span>
                  {t.recorrencia && <Repeat className="w-3 h-3 text-zinc-500 shrink-0" />}
                </div>
                <div className="flex items-center gap-2.5 text-[11px] text-zinc-500 mt-0.5 num-tabular flex-wrap">
                  <span>
                    {diaSemanaCurto(t.data_prazo)} {dataCurta(t.data_prazo)}
                  </span>
                  {t.hora_prazo && (
