...
/**
 * Converte números por extenso (PT-BR) pra dígitos. Necessário porque o
 * Whisper transcreve fala natural ("cento e trinta e cinco") em vez de
 * dígitos, e o regex de valor só pega "135".
 *
 * Suporta: zero a trilhões. Composto: "duzentos e cinquenta e três",
 * "mil duzentos", "dois mil e quinhentos", "um milhão e cem".
 *
 * Limitação: cobre os casos comuns de áudio curto de WhatsApp
 * (valores até ~100 mil). Não tenta ser exaustivo — fora isso,
 * deixa o Groq resolver.
 */
function numerosPorExtensoParaDigitos(texto: string): string {
  ...
  // Compostos especiais: "cento e N" (101-199) usa "cento" + (1-99)
  // (cem sozinho = 100; cento e um = 101)
